...
class CartViewController: UIViewController {
    ...
    @objc @IBAction func checkoutButtonTapped() {
        ...
        let alert = UIAlertController(
            ...
            message: String(format: "Подтвердить заказ на сумму %.2f ₽?", CartManager.shared.totalPrice),
...
extension CartViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView,
        ...
        content.secondaryText = "\(item.duration) • \(String(format: "%.2f ₽", item.price))"
...
extension CartViewController: UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView,
        ...
        let item = CartManager.shared.cartItems[indexPath.row]
        showMembershipDetails(item)
    }

    // MARK: - Helper Methods
    private func showDeleteConfirmation(for item: Membership,
    ...
    private func showMembershipDetails(_ item: Membership) {
        let alert = UIAlertController(
            title: item.title,
